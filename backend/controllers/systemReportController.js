...
export const generateSystemReport = async (req, res) => {
  ...
  getAdminEmail().then(adminEmail => {
    ...
    const depH = dependencies.length ? dependencies.map(d => `<li><b>${d.name}</b> (${d.source}): ${d.current} → ${d.latest} [${d.type}]</li>`).join("") : "<li>✅ All up to date</li>";
    const secH = security.length ? security.map(s => `<li><b>${s.package}</b> — ${s.severity}: ${s.issue}</li>`).join("") : "<li>✅ No vulnerabilities</li>";
    const errH = errors.length ? errors.slice(0, 20).map(e => `<li><code>${e.file}:${e.line}</code> [${e.ruleId}] ${e.message}</li>`).join("") : "<li>✅ No errors</li>";
    const wrnH = warnings.length ? warnings.slice(0, 20).map(w => `<li><code>${w.file}:${w.line}</code> [${w.ruleId}] ${w.message}</li>`).join("") : "<li>✅ No warnings</li>";
    sendMail(adminEmail,
      `[StayOS] System Audit — ${new Date().toLocaleDateString("en-IN")}`,
      `<h2>🔐 StayOS System Audit Report</h2>
       <p><b>Generated:</b> ${new Date().toLocaleString("en-IN")}</p>
       <h3>📦 Dependencies (${dependencies.length} outdated)</h3><ul>${depH}</ul>
       <h3>🔐 Security (${security.length} issues)</h3><ul>${secH}</ul>
       <h3>❌ Errors (${errors.length})</h3><ul>${errH}</ul>
       <h3>⚠️ Warnings (${warnings.length})</h3><ul>${wrnH}</ul>
       <p>Login to your admin portal to review and take action.</p>`
    );
  });
